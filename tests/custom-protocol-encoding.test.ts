...
describe('カスタムプロトコルのURLエンコード/デコード', () => {
  describe('getCustomProtocolUrl', () => {
    test('空白を含むファイルパスが正しくエンコードされる', () => {
      ...
      expect(result).toBe('komae-asset://%2FUsers%2Ftest%2Fprojects%2Ftest%20project%2Fassets%2Fimages%2Ftest%20image.jpg');
    });
    ...
    test('空白を含む日本語ファイルパスが正しくエンコードされる', () => {
      ...
      // 空白文字が%20にエンコードされることを確認
      expect(result).toBe('komae-asset://%2FUsers%2Fgreymd%2FDesktop%2Faaa%20bbb%2Fassets%2Fimages%2F1-18.jpg');
    });
    ...
    test('通常のファイルパス（空白なし）は正常に処理される', () => {
      ...
      expect(result).toBe('komae-asset://%2FUsers%2Ftest%2Fprojects%2Fnormal-project%2Fassets%2Fimages%2Fnormal-image.jpg');
    });
  });
