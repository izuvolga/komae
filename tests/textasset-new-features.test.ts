/**
 * TextAsset新機能のテスト
 * Phase 2B: default_language_settings, override_language_settings機能
 */

import {
  createDefaultTextAsset,
  migrateTextAssetToNewFormat,
...
describe('TextAsset New Features', () => {
  ...
  describe('migrateTextAssetToNewFormat', () => {
    it('should migrate old TextAsset to new format', () => {
      const oldAsset: TextAsset = {
        id: 'test-1',
        type: 'TextAsset',
        name: 'Old Text',
        default_text: 'Hello',
        ...
        vertical: false,
        default_z_index: 1,
      } as TextAsset; // Cast to avoid missing new fields error

      const supportedLanguages = ['ja', 'en'];
      const migratedAsset = migrateTextAssetToNewFormat(oldAsset, supportedLanguages);
      
      expect(migratedAsset.default_context).toBe('');
      expect(migratedAsset.default_language_settings).toBeDefined();
      expect(Object.keys(migratedAsset.default_language_settings!)).toEqual(supportedLanguages);
    });

    it('should not migrate already migrated TextAsset', () => {
      const newAsset = createDefaultTextAsset({ 
        name: 'New Text',
        supportedLanguages: ['ja', 'en'] 
      });
      
      const migratedAsset = migrateTextAssetToNewFormat(newAsset, ['ja', 'en']);
      
      expect(migratedAsset).toBe(newAsset); // Same reference, no changes
    });
  });
  ...
  describe('Backward Compatibility', () => {
    it('should still work with old multilingual_overrides format', () => {
      const testAsset: TextAsset = {
        id: 'test-1',
        type: 'TextAsset',
        name: 'Legacy Text',
      ...
      const legacyInstance: TextAssetInstance = {
        id: 'legacy-instance',
        asset_id: testAsset.id,
        multilingual_text: { 'ja': 'テスト' },
      ...
      const fontSize = getEffectiveFontSize(testAsset, legacyInstance, 'ja');
      expect(fontSize).toBe(18); // From legacy override

      const position = getEffectivePosition(testAsset, legacyInstance, 'ja');
      expect(position.x).toBe(50); // From legacy override
      expect(position.y).toBe(100); // From asset default
    });
  });
});