...
Available Variables として、ValueAsset の値をそのまま変数として利用できる。ValueAsset の name に対応する。
`value_1`, `value_2`, ... が例示として挙げられている。

`[▶Run]` ボタンをクリックすると、Script の内容が実行され、返り値の SVG がプレビューに表示される。
