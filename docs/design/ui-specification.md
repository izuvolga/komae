...
「[ ] 縦横比を元画像にあわせる」のチェックボックスは、チェックを入れると、Default Width/Height の値を変更した際に、元画像の縦横比を維持するように自動的に計算される。
Default PosX/PosY は、画像のデフォルトの位置を指定する項目で、ここで指定した値が ImageAsset のデフォルト値として保存される。
...
- 例えば
  ...
  - 各セルにある `[x]` はチェックボックスで、その AssetInstance の表示の ON/OFF を切り替えらえっる。
