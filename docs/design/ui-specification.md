...
初期状態 から E を1の位置に挿入すると、以下のように、Eが一番左に移動する。
...
初期状態 から E を2の位置に挿入すると、以下のように、EがAとBの間に移動する。
...
初期状態 から E を3の位置に挿入すると、以下のように、EとDの位置が入れ替わる
...
また、非表示にされている連続している連続している列は、再表示しない限り、挿入位置として認識されない。
