...
- [ ]: プロジェクト作成ページの改善
  ...
  - [x]: キャンバスサイズ選択するときのアイコンと実際のサイズがあってない
  - キャンバスのデフォルトの色を選択できるようにする
  - [x]: 対応言語の横のヘルプアイコンをホバーにする
  - [x]: プロジェクト名の横にヘルプアイコンをつけて以下の説明をする
...
- [ ]: ValueAsset 改善
  - color 型対応
  - string 型の多言語対応
- [x]: プレビューエリアの背景が透明になってる？
- [x]: 通知がモーダルの黒い背景の裏側にある
- [x]: 新規プロジェクトを作成したとき、プレビューエリアの拡大方法がズレている。
- [x]: アプリ設定画面にダークモードのトグルを追加
- [x]: アプリ設定画面をツールバーから参照させるようにし、Cmd + , で開けるようにする
- [x]: ツールバーのタイトルを Electron ではなくアプリ名にする。 => 開発中は無理
- [ ]: hiddenColumns, hiddenRows は komae ファイルではなく、別の箇所に保存
  - ローカルストレージ
  - settings.yaml みたいなファイル
