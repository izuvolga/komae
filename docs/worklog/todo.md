...
- [x]: TextAssetInstance の Edit 画面で「縦書き」および「フォント」を各言語で override できるようにする
  - TextEditModal も編集する
- [x]: Preview Window の「自動ズーム」と「キャンバスをプレビュー画面に収める」を統合する
...
- [x]: Project Header のプルダウンメニューのデザインがダサいので改善する
  - `P:🖊️` や `[JA|▼]` や `T:🖊️` の表記があるが、更新したい
    ...
    - また、プルダウンできることを示す四角形で囲まれた▼のようなボタンも設置する
...
- [x]: TrueType/OpenType フォントを読み込んだときに、TrueType/OpenType のメタ情報を取得して、Full name の情報を自動的にフォント名として設定する。
...
- [x]: fonts-registry.json に存在しないフォントはデータ不整合なので自動削除するようにする
- [x]: Google フォントのフォントを追加する機能を実装する
